...
class LRFinder(object):
    ...
    def __init__(self, model, optimizer, criterion, device=None, memory_cache=False, cache_dir=None):
        self.model = model
        ...
        for iteration in tqdm(range(num_iter)):
            ...
            if iteration == 0:
                ...
                if loss < self.best_loss:
                    self.best_loss = loss
                    self.best_lr = lr_schedule.get_lr()[0]
    ...
    def plot(self, skip_start=10, skip_end=5, log_lr=True):
        ...
        print("The best loss was {:.2f} at a learning rate of {}".format( self.best_loss, self.best_lr))
        plt.plot(lrs, losses)
        if log_lr:
            plt.xscale("log")
